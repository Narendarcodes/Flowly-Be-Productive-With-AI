...
const configsBuilder = ({ matchesDir, srcDir, rootDir, contentName }) => Object.entries(getContentScriptEntries(matchesDir)).map(([name, entry]) => ({
    name,
...
const builds = async ({ srcDir, contentName, rootDir, matchesDir, withTw }) => configsBuilder({ matchesDir, srcDir, rootDir, contentName }).map(async ({ name, config }) => {
    if (withTw) {
...
export const contentBuilder = async ({ matchesDir, srcDir, rootDir, contentName, withTw = true, }) => builds({
    srcDir,
    contentName,
    rootDir,
    matchesDir,
